...
import locationBackground from "../../assets/images/background/location-background.webp";
...
const SetupYourLocationPage = () => {
  ...
  const validate = () => {
    ...
    if (!localData.country) newErrors.country = "Please select a country";
    if (!localData.district.trim()) newErrors.district = "District is required";
    if (!localData.division.trim()) newErrors.division = "Division is required";
    if (!localData.postalCode.trim()) {
      newErrors.postalCode = "Postal Code is required";
    } else if (!/^[0-9]{4,10}$/.test(localData.postalCode.trim())) {
  ...
  const progress =
    ...
    (localData.country ? 1 : 0) +
    (localData.district ? 1 : 0) +
    (localData.division ? 1 : 0) +
    (localData.postalCode ? 1 : 0);
      ...
      <div className="flex flex-col lg:flex-row gap-10 lg:gap-20 px-4 md:px-10 relative z-10">
        ...
        <div className="flex-1 max-w-lg text-center lg:text-left animate-fadeInUp">
          <h1 className="font-hugiller text-[60px] sm:text-[90px] md:text-[110px] lg:text-[120px] text-[#263D5D] leading-[1.1] mb-4">
            Setup your <br />
            <span className="text-[#3ABBD0] relative">Location</span>
          </h1>
